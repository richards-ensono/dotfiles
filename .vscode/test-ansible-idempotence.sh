...
for command_name in fd govulncheck staticcheck gosec actionlint; do
  ...
  if [ -z "$command_path" ]; then
    echo "$command_name was not found on PATH after the first playbook run" >&2
    exit 1
  fi
  ...
  if ! "$command_path" "$version_argument" >/dev/null 2>&1; then
    echo "$command_name command at $command_path did not execute successfully after the first playbook run" >&2
    exit 1
  fi

  echo "$command_name available at $command_path"
done
